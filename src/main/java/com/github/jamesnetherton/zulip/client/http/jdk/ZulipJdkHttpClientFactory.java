package com.github.jamesnetherton.zulip.client.http.jdk;

import com.github.jamesnetherton.zulip.client.exception.ZulipClientException;
import com.github.jamesnetherton.zulip.client.http.ZulipConfiguration;
import com.github.jamesnetherton.zulip.client.http.ZulipHttpClient;
import com.github.jamesnetherton.zulip.client.http.ZulipHttpClientFactory;

/**
 * A {@link ZulipHttpClientFactory} implementation that creates a {@link ZulipHttpClient} based on
 * the JDK HTTP Client.
 */
public class ZulipJdkHttpClientFactory implements ZulipHttpClientFactory {

    @Override
    public ZulipHttpClient createZulipHttpClient(ZulipConfiguration configuration) throws ZulipClientException {
        return new ZulipJdkHttpClient(configuration);
    }
}
